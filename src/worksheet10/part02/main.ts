...
const main= async() =>
{
  ...
  const z : Float32Array = vec3.subtract(vec3.create(0, 0, radius), target);
  ...
  function render () : void {
    const eye : Float32Array = vec3.add(vec3.transformQuat(vec3.create(0, 0, radius), quatRot), target);
    mat4.lookAt(eye,target, vec3.transformQuat(up, quatRot), view);
    ...
    device.queue.writeBuffer(uniformBuffer, mvp.byteLength * 2, new Float32Array([0.0, 0.0, radius]));
    ...
    device.queue.writeBuffer(uniformBufferQuad, 64 * 2, new Float32Array([0.0, 0.0, radius]));
